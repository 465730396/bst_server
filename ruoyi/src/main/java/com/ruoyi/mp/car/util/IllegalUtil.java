...
//    private static String appcode = "2d5ed842d99c492f94a3b663755c3556";
    ...
    public static IllegaResponse getIllegaData(String lsprefix, String lsnum, String engineno, String frameno){
        try{
//            IllegaResponse illegaResponse = null;
//            HttpResponse response = getIllegaDataByFree(lsprefix,lsnum,engineno,frameno);
//            S4SResponse data = JSON.parseObject(EntityUtils.toString(response.getEntity()), S4SResponse.class);
//            if (data == null){
//                illegaResponse = JSON.parseObject(EntityUtils.toString(response.getEntity()),IllegaResponse.class);
//            }
            HttpResponse response = getIllegaDataByPay(lsprefix,lsnum,engineno,frameno);
    ...
    private IllegaDetail convertData(S4SRecord record){
        IllegaDetail detail = new IllegaDetail();
        detail.setTime(record.getOccTime());
        detail.setAddress(record.getAddress());
        detail.setContent(record.getReason());
        detail.setLegalnum(record.getViolationCode());
        detail.setPrice(record.getFine());
        detail.setScore(record.getPoint());
        detail.setNumber(record.getViolationId());
//        detail.setAgency();
        detail.setHandlefee(record.getFree());
        return detail;
    }
    private IllegaResult IllegaResult(S4SData s4SData){
        IllegaResponse response = new IllegaResponse();
        response.setMsg(s4SData.getMsg());
        response.setStatus(s4SData.getCode()==1?0:1);

        IllegaResult result = new IllegaResult();
        result.setCount(String.valueOf(s4SData.getTotal()));
        result.setTotalscore(String.valueOf(s4SData.getDegree()));
        result.setTotalprice(String.valueOf(s4SData.getFine()));

        List<IllegaDetail> list = new ArrayList<>();
        for (S4SRecord record : s4SData.getRecords()){
            IllegaDetail illegaDetail = convertData(record);
            list.add(illegaDetail);
        }
        result.setList(list);
